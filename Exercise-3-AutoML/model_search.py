...
class BigramLanguageModel(nn.Module):
    ...
    def sample_random_config(self):
        ...
        config["num_layers"] = np.random.choice(num_layers, size=1)
        config["embed_dim"] = np.random.choice(embed_dims, size=1)
        config["num_heads"] = np.random.choice(num_heads, size=config["num_layers"])
        config["mlp_ratio"] = np.random.choice(mlp_ratio, size=config["num_layers"])

        arch_param_dict["num_layers"] = torch.zeros(len(num_layers), requires_grad=True)
        arch_param_dict["embed_dim"] = torch.zeros(len(embed_dims), requires_grad=True)
        arch_param_dict["num_heads"] = torch.zeros(max(num_layers), len(num_heads), requires_grad=True)
        arch_param_dict["mlp_ratio"] = torch.zeros(max(num_layers), len(mlp_ratio), requires_grad=True)

        # This gives the precise one-hot encoding according to the selected config
        for key in self.choices:
            values_choices = self.choices[key]
            values_config = config[key]
            values_arch_param = arch_param_dict[key]
            for i in range(len(self.choices[key])):
                for j in range(len(config[key])):
                    if values_choices[i] == values_config[j]:
                        if len(values_arch_param.shape) > 1:
                            values_arch_param.data[j, i] = 1.0
                        else:
                            values_arch_param.data[i] = 1.0

        # If the max num of layers is not the randomly selected config, it will fill out the rest of the arch param
        # matrix with dummy values at the first location that will not affect the procedure loss and results
        if max(num_layers) != config["num_layers"]:
            values_arch_param_num_layers = arch_param_dict["num_heads"]
            values_arch_param_mlp_ratio = arch_param_dict["mlp_ratio"]
            for i in range((max(num_layers) - config["num_layers"].item())):
                values_arch_param_num_layers.data[-(i+1), 0] = 1.0
                values_arch_param_mlp_ratio.data[-(i+1), 0] = 1.0

        # Because working with np.arrays is easier for me and faster in general, I used np.arrays in the code above, but
        # for the evo search this will not work, hence here I change the dictionary making the code for the next excecise work
        converted_config = {}
        for key, value in config.items():
            if key in ['num_layers', 'embed_dim']:
                converted_config[key] = int(value)
            else:
                converted_config[key] = value.tolist()
