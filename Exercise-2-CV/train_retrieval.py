...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        similarity_image_text = (image_feat @ text_feat.T) / temperature
        similarity_text_image = (text_feat @ image_feat.T) / temperature
        # 2. Define the one-hot targets, an identity matrix (take care to select the correct device
        # and dtype)
        targets_image_text = torch.eye(similarity_image_text.size(0)).to(device)
        targets_text_image = torch.eye(similarity_text_image.size(0)).to(device)
        # 3. Compute the image-to-text and text-to-image cross-entropy loss.
        image_to_text_loss = loss_fn(similarity_image_text, targets_image_text)
        text_to_image_loss = loss_fn(similarity_text_image, targets_text_image)
