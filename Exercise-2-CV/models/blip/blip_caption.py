...
class BlipCaption(BlipBase):
    ...
    def greedy_search(
        ...
        while True:
            ...
            output_logits = self.text_decoder(input_ids=input_ids, attention_mask=attention_mask, 
    ...
    def sampling(
        ...
        while True:
            ...
            # Unable to get the advised results for this task... But further explanation in the report
            # I have also tried 2 different strategies which seem to always get similar results
            # I am not sure if there is a correction for this task but i am very eager to know
            # what the solution actually is for this because i spent alot of time on it with no good results
            scaled_tokens_logits = next_tokens_logits/temperature
            # Strategy 0:
            
            # topk_logits, topk_indices = torch.topk(scaled_tokens_logits, topk, dim=-1)
            # next_token_probs = F.softmax(topk_logits, dim=-1)
            # next_tokens = torch.multinomial(next_token_probs, num_samples=1)
            # next_tokens_caption = torch.gather(topk_indices, dim=-1, index=next_tokens)

            # Strategy 1:

            tokens_prob = F.softmax(input=scaled_tokens_logits, dim=-1)
            topk_prob, topk_indices = torch.topk(input=tokens_prob, k=topk, dim=-1)
            sum_topk_prob = torch.sum(topk_prob, dim=1, keepdim=True)
            renormalized_topk_prob = topk_prob / sum_topk_prob
            next_tokens = torch.multinomial(input=renormalized_topk_prob, num_samples=1, replacement=True)
            next_tokens_caption = torch.gather(input=topk_indices, dim=-1, index=next_tokens)

            input_ids = torch.cat([input_ids, next_tokens_caption], dim=-1) 
            # raise NotImplementedError
            # END TODO ###################    
