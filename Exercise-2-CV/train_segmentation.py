...
# I commented all the things I used for tensorboard and image producing
...
def main(args):
    ...
    if args.decoder_type == "linear":
        ...
        # Writer = SummaryWriter(log_dir='./Tensorboard/val_segmentation_linear')
    elif args.decoder_type == "convolutional":
        # START TODO #################
        # implement a small convolutional decoder

        # Here I made 2 different implementation for the convolutional head to see hpw both can affect the result

        # decoder = nn.Sequential(
        #     nn.Conv2d(in_channels=encoder.embed_dim, out_channels=(encoder.embed_dim//2), kernel_size=3, padding=1),
        #     nn.Conv2d(in_channels=(encoder.embed_dim//2), out_channels=len(training_dataset.classes), kernel_size=3, padding=1)
        # )

        decoder = nn.Conv2d(in_channels=encoder.embed_dim, out_channels=len(training_dataset.classes), kernel_size=3, padding=1)
        
        # Writer = SummaryWriter(log_dir='./Tensorboard/val_segmentation_conv')
        ...
        decoder = TransformerSegmentationHead(num_classes=len(training_dataset.classes), dim=encoder.embed_dim,
        num_heads=encoder.num_heads, shared_qk=args.transformer_head_shared_qk) 
        # Writer = SummaryWriter(log_dir='./Tensorboard/transformer_shared')
        # raise NotImplementedError
        # END TODO ###################
    else:
        raise ValueError
    ...
    for epoch in range(args.epochs):
        ...
        # average_ious = np.nanmean(ious)*100
        # Writer.add_scalar('Validation Accuracy', average_ious, global_step=step)
        # step += 1
...
def validate(model, data_loader):
    ...
    for idx, sample in enumerate(data_loader):
        ...
        # if idx == 0:
        #     seg_viz.draw_and_save(Image.open(sample["img_path"][0]), logits.argmax(1), "output1.png")
        # if idx == 1:
        #     seg_viz.draw_and_save(Image.open(sample["img_path"][0]), logits.argmax(1), "output2.png")
        # if idx == 2:
        #     seg_viz.draw_and_save(Image.open(sample["img_path"][0]), logits.argmax(1), "output3.png")
